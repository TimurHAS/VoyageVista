...
try { switch ($action) {
    ...
    case 'confirm':
        ...
        // Récupère le panier ouvert
        $cartStmt = $db->prepare('SELECT * FROM carts WHERE user_id = :u AND status = "open" LIMIT 1');
        $cartStmt->execute([':u' => $userId]);
        $cart = $cartStmt->fetch();

        if ($cart) {
            // Sauvegarde dans reservations
            $resStmt = $db->prepare('INSERT INTO reservations
                (user_id, destination_id, transport_id, hotel_id, activity_ids,
                 adults, children, nights, bags, ticket, seat, room_type, hotel_options, checkin, checkout)
            ...
            $resStmt->execute([
                ...
                ':seat'   => $cart['seat'],
                ':room'   => $cart['room_type'],
                ':hopt'   => $cart['hotel_options'],
                ':ci'     => $cart['checkin'],
                ':co'     => $cart['checkout'],
            ]);

            // Vide le panier après confirmation
            $db->prepare('DELETE FROM carts WHERE user_id = :u')->execute([':u' => $userId]);
        }
